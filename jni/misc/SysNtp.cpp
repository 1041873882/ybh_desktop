	...
	if (FD_ISSET(s, &rfds)) {
		...
		if (ret > 0) {
			...
			if (labs(time(NULL)-sec) > 3) {
				tv.tv_sec = sec;
				tv.tv_usec = 0;
				settimeofday(&tv, NULL);	/* 同步系统时间，时区为UTC+0 */
...
// #define NTP_TIMEOUT	(5*60)		/* 与本地时间相差5分钟进行一次NTP服务查询 */
#define NTP_TIMEOUT	(5)	//10s查询	YBH 2020 8/25
void sys_ntp_process(void)
{
	...
	if (labs(time(NULL)-ts) >= NTP_TIMEOUT) {
		if (do_ntp(sys.talk.server(), 9123)) {
			if (sys.net.ntp_enable()) {			// YBH 2020 8/24 ntp使能标志位
