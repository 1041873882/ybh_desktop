...
wSetupNormalLabel::wSetupNormalLabel() : mWindow("setup/normal")
{
	...
	m_ring_text.setParent(this);
	m_ring_text.load(m_style, "ring_text");
	m_ring.setParent(this);
	m_ring.load(m_style, "ring");
	m_ring.insert(m_style.getText("/style/text/ring0"));
	m_ring.insert(m_style.getText("/style/text/ring1"));
	m_ring.insert(m_style.getText("/style/text/ring2"));
	m_ring.insert(m_style.getText("/style/text/ring3"));
	m_ring.select(sys.user.ring());

	m_ban_text.setParent(this);
	m_ban_text.load(m_style, "ban_text");
	m_ban.setParent(this);
	m_ban.load(m_style, "ban");
	m_ban_st = sys.user.ban();
	if (m_ban_st)
		m_ban.setText("√");
	else
		m_ban.setText("");
	...
	if (e->type == mEvent::TouchEnd) {
		if (e->wParam == m_ok.id()) {
			...
			if (m_answer_st)
				...
				m_answer.setText("");
		} else if (e->wParam == m_ban.id()) {
			m_ban_st = m_ban_st ? 0 : 1;
			if (m_ban_st)
				m_ban.setText("√");
			else
				m_ban.setText("");
		}
	} else if (e->type == mEvent::ComboxEnd) {
		if (e->wParam == m_ring.id()) {
			sound.stop();
			int n = m_ring.select();
			sound.play(sound.m_ring[n], 0);
		}
	}
}
